...
#--------------------------------------------------------------------------
#  Rota de DELETE — Apaga/sinaliza o cliente 
#--------------------------------------------------------------------------
#from .seguranca import validar_api_key  # ajuste o import conforme seu projeto

@router.delete("/cliente/{cli_codigo}", dependencies=[Depends(validar_api_key)])
def excluir_cliente(
    cli_codigo: str,
    hard: bool = Query(False, description="Se true, remove definitivamente (hard delete). Padrão: false (soft delete).")
):
    """
    Exclui um cliente.
    - soft delete (padrão): seta flags de inativação
    - hard delete: apaga o registro
    """
    conn = get_conexao()
    cur = conn.cursor(cursor_factory=RealDictCursor)

    try:
        # 1) Confere existência
        cur.execute("""
            SELECT cli_codigo, cli_nome, COALESCE(cli_ativo, 1) AS cli_ativo
            FROM clientes
            WHERE cli_codigo = %s
            LIMIT 1
        """, (cli_codigo,))
        ...
        if hard:
            # 2A) HARD DELETE
            try:
                cur.execute("DELETE FROM clientes WHERE cli_codigo = %s", (cli_codigo,))
                apagadas = cur.rowcount
                conn.commit()
                return {
                    "status": "ok",
                    "modo": "hard",
                    "mensagem": f"Cliente {cli_codigo} removido definitivamente.",
                    "linhas_afetadas": apagadas
                }
            except errors.ForeignKeyViolation:
                conn.rollback()
                # Conflito por integridade referencial
                raise HTTPException(
                    status_code=409,
                    detail="Não é possível excluir: existem registros relacionados (FK)."
                )

        else:
            # 2B) SOFT DELETE
            # Ajuste os nomes das colunas de flags conforme seu schema
            # Ex.: cli_ativo (int/boolean), cli_desativar_sistema (int/boolean), data_inativacao (date)
            cur.execute("""
                UPDATE clientes
                   SET cli_ativo = 0,
                       cli_desativar_sistema = 1,
                       cli_data_inativacao = CURRENT_DATE
                 WHERE cli_codigo = %s
                 AND (cli_ativo IS DISTINCT FROM 0)
            """, (cli_codigo,))
            atualizadas = cur.rowcount
            conn.commit()

            # Se já estava inativo, ainda consideramos a operação idempotente
            return {
                "status": "ok",
                "modo": "soft",
                "mensagem": f"Cliente {cli_codigo} marcado como inativo.",
                "ja_estava_inativo": (atualizadas == 0)
            }
